...
export class AuthService {
  ...
  constructor() {
    if (!process.env.SIMA_BASE_URL) {
      throw new Error(
        "SIMA_BASE_URL environment variable is required. Please set it in your .env file."
      );
    }
    ...
    try {
      ...
      if (response.status === 302 || response.status === 303) {
        ...
        if (redirectLocation && redirectLocation.includes("testsession=")) {
          ...
          if (
            testSessionResponse.status === 302 ||
            testSessionResponse.status === 303
          ) {
            const finalRedirect = testSessionResponse.headers["location"];
            if (finalRedirect && !finalRedirect.includes("testsession=")) {
              console.log(
                "🎯 TestSession auto-redirecting to dashboard:",
                finalRedirect
              );

              const testSessionCookies =
                testSessionResponse.headers["set-cookie"] || [];
              ...
              console.log("🎯 Dashboard response:", {
                ...
                url:
                  dashboardResponse.request?.res?.responseUrl ||
                  dashboardResponse.config.url,
          ...
          console.log(
            "🔗 TestSession final URL:",
            testSessionResponse.request?.res?.responseUrl ||
              testSessionResponse.config.url
          );
          console.log(
            "📊 TestSession final status:",
            testSessionResponse.status
          );
          console.log(
            "🍪 TestSession final cookies count:",
            testSessionCookies.length
          );

          // Check if TestSession redirects us to the dashboard
          const testSessionFinalUrl =
            testSessionResponse.request?.res?.responseUrl ||
            testSessionResponse.config.url;
          ...
          if (testSessionFinalUrl?.includes("testsession=")) {
            console.log(
              "🔄 TestSession still at testsession URL, checking for dashboard redirect..."
            );
            ...
            try {
              ...
              console.log(
                "🍪 Dashboard cookies count:",
                dashboardCookies.length
              );
              console.log(
                "🍪 Dashboard cookies:",
                dashboardCookies.slice(0, 3)
              );

              // Check if we got MoodleSession or similar authentication cookies
              const hasMoodleSession = completeCookies.some(
                (cookie) =>
              ...
              console.log(
                "🔑 Got MoodleSession-like cookies:",
                hasMoodleSession
              );
  ...
  async validateSession(cookies: string[]): Promise<boolean> {
    try {
      console.log(
        "🔍 Validating session with cookies:",
        this.parseCookies(cookies).substring(0, 100) + "..."
      );
      ...
      console.log(
        "📄 Session validation - Response contains form?",
        response.data?.includes("<form")
      );
      console.log(
        "📄 Session validation - Response contains username field?",
        response.data?.includes("username")
      );
      console.log(
        "📄 Session validation - Response contains MoodleSession?",
        response.data?.includes("MoodleSession")
      );

      const isRedirectedToLogin =
        response.request?.res?.responseUrl?.includes("/login");
      const hasLoginForm =
        response.data?.includes("<form") && response.data?.includes("username");
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (error.response?.status === 302 || error.response?.status === 303) {
          ...
          console.log(
            "✅ ValidateSession redirect validation result:",
            isValid
          );
          return isValid;
        }
