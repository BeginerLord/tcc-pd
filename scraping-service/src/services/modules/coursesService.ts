...
export class CoursesService {
  ...
  async getUserCourses(cookies: string[]): Promise<CourseInfo[]> {
    try {
      ...
      for (const testUrl of testUrls) {
        try {
          ...
          const hasUsernameField =
            ...
            response.data?.includes('id="username"');
          const hasPasswordField =
            response.data?.includes('name="password"') ||
            response.data?.includes('id="password"');
      ...
      $(".course-info-container, .coursebox, [data-course-id]").each(
        (index, element) => {
          const $course = $(element);
          const courseId =
            $course.attr("data-course-id") ||
            $course.find("[data-course-id]").attr("data-course-id") ||
            $course
              .find('a[href*="/course/"]')
              .attr("href")
              ?.match(/course\/view\.php\?id=(\d+)/)?.[1];

          const courseName =
            $course
              .find(".coursename, .course-title, h3")
              .first()
              .text()
              .trim() || $course.find('a[href*="/course/"]').text().trim();

          const shortName =
            $course.find(".course-shortname, .shortname").text().trim() ||
            courseName.split(" ")[0];
